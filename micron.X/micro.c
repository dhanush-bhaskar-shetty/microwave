...
void set_time(unsigned char key,unsigned char reset_flag){
    static unsigned char blink_pos=0;//0 indicated we will blink seconds field 
    ...
    //read the time from keyboard 
    if(key != '*'&& key !='#' && key !=ALL_RELEASED){
        key_count++;// <2 for sec , >2 is min
        if(key_count<=3){
            sec=sec*10 + key;
            blink_pos=0;//update to  seconds position   
        }else if(key_count<=5){
            min=min * 10 +key;
            blink_pos=1;
...
void set_temp(unsigned char key,unsigned char reset_flag){
    ...
    if(key != '*'&& key !='#' && key !=ALL_RELEASED){
        key_count++;
        if(key_count<=4){ //3 times temperature is read
    ...
    //# start cooking with time set display the time 
        ...
        clcd_print("Time Rem:",LINE2(0));
        clcd_putch( 's',LINE3(12));
        while(preheat){
            clcd_putch(preheat/100 +'0',LINE3(9));
            clcd_putch((preheat/10)%10 +'0',LINE3(10));
            clcd_putch((preheat%10) +'0',LINE3(11));
